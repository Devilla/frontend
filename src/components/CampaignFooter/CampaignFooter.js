  ...
  return (
    <div className="campaign-footer-container">
      <div className="campaign-footer-steps">
        ...
        {notification && <button type="button" className="text-center btn-footer-2 mr-1 influence-default-button" onClick={setDefault}>Default</button>}
        {notification && <button type="button" className="btn-right btn-footer-3 ml-1" onClick={saveConfigure}>Save</button>}
        {!notification && <button type="button" className="text-center btn-footer-2 mr-1 influence-next-button" onClick={() => step == 5?'':handleNextState(step+1)}>{step == 5?'Finish':'Next'}</button>}
        {!notification && <button type="button" className="btn-right btn-footer-3 ml-1" data-toggle="modal" data-target="#myModallive" onClick={goLive}>Publish</button>}
      </div>
    </div>
  );
};
