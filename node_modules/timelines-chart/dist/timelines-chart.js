...
function continuous$1(deinterpolate, reinterpolate$$1) {
  ...
  scale.invert = function(y) {
    return (input || (input = piecewise(range, domain, deinterpolateLinear$1, clamp ? reinterpolateClamp$1(reinterpolate$$1) : reinterpolate$$1)))(+y);
  };
...
function continuous$2(deinterpolate, reinterpolate$$1) {
  ...
  function scale(x) {
    return (output || (output = piecewise(domain, range, clamp ? deinterpolateClamp$2(deinterpolate) : deinterpolate, interpolate$$1)))(+x);
  }

  scale.invert = function(y) {
    return (input || (input = piecewise(range, domain, deinterpolateLinear$2, clamp ? reinterpolateClamp$2(reinterpolate$$1) : reinterpolate$$1)))(+y);
  };

  scale.domain = function(_) {
    return arguments.length ? (domain = map$4.call(_, number$6), rescale()) : domain.slice();
  };
...
var gradient = Kapsule({
    props: {
        ...
        colorScale: { default: linear$4().range(['black', 'white']) },
    ...
    update: function update(state) {
        ...
        var stopsScale = linear$4().domain([0, 100]).range(state.colorScale.domain());
